...
| 8.6 fake smoke | Done | Fake smoke validates the approval-to-runner-to-quality-to-learning path without a real CLI provider. |
| 8.7 live smoke | Manual | Use the live smoke script only on machines with an authenticated CLI provider. Do not make it a mandatory automated gate. |
| 8.8 Phase 7 boundary | Done | Orchestration/pipeline tests cover plan approval, worker execution, downstream approvals, and remote endpoint routing without bypassing approval policy. |
| 8.9 post-Phase 8 serverless boundary | Done | A2A follow-up is outbound/client-side from the desktop perspective; inbound serving remains out of scope. |
...
- If `better-sqlite3` reports a Node ABI mismatch, close the running Electron
  app and run `npm run rebuild:node` before smoke scripts.
