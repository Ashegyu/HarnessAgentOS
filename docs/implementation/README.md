...
이 문서 세트는 HarnessAgentOS를 Phase 0부터 Phase 8까지 순서대로 구현하기 위한 실행 설계다. 기준 설계서는 제품 철학과 전체 구조를 고정하고, 이 구현 문서 세트는 실제 작업자가 어떤 순서로 어떤 모듈, 타입, IPC, DB, UI, 테스트를 만들어야 하는지 결정 완료 상태로 제공한다.

구현자는 별도 아키텍처 결정을 새로 하지 않는다. Phase 문서의 완료 기준을 만족한 뒤 다음 Phase로 넘어간다.
...
2. `docs/implementation/phase-00-foundation.md`
3. `docs/implementation/phase-01-local-state-model.md`
4. `docs/implementation/phase-02-conversation-to-approval.md`
5. `docs/implementation/phase-03-runner-and-artifacts.md`
6. `docs/implementation/phase-04-quality-gates.md`
7. `docs/implementation/phase-05-skillify-capability-adapter.md`
...
앱 scaffold/build/test 실행은 실제 구현 단계에서 수행한다. 이 문서 세트는 구현 기준이다.
