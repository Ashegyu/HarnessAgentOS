...
const main = async () => {
  // 1. Happy path — file_write action is accepted, then the full
  //    approval → runner → quality → done → LearningTrace chain is closed.
  await run("ok-file-write", async (ctx) => {
    ...
    const targetFile = join(ctx.projectDir, "README.smoke.md");
    ...
    expect(!existsSync(targetFile), "approval did not write before execution");

    const approval = result.approvals[0];
    expect(Boolean(approval?.proposedAction), "approval carries proposedAction");
    const approved = await ctx.conversation.approve({
      approvalId: approval.id,
    });
    expect(approved.status === "approved", "approval approved");
    const runnerResult = await ctx.runner.executeApproved(approval.id);
    expect(
      runnerResult.changedFiles?.some((p) => p.endsWith("README.smoke.md")) ??
        false,
      "runner reports changed file",
    );
    expect(existsSync(targetFile), "runner wrote approved file");
    expect(
      readFileSync(targetFile, "utf8").includes("Generated by FakeModelCliAdapter"),
      "written file contains fake adapter content",
    );

    const gate = await ctx.qualityEvaluator.evaluate({
      taskRunId: draft.taskRun.id,
    });
    expect(
      gate.status === "warning" || gate.status === "passed",
      `quality gate evaluated (${gate.status})`,
    );
    await ctx.qualityCompletion.applyQualityGateResult(gate);
    if (gate.status === "warning") {
      await ctx.qualityCompletion.approveKnownRisks({
        taskRunId: draft.taskRun.id,
        message: "Fake smoke accepts missing test evidence for generated fixture content.",
      });
    }
    const done = await ctx.qualityCompletion.markDone({
      taskRunId: draft.taskRun.id,
    });
    expect(done.status === "done", "markDone reached done");
    const trace = await ctx.state.getLearningTraceByTaskRun(draft.taskRun.id);
    expect(trace?.success === true, "LearningTrace recorded successful outcome");
  });
